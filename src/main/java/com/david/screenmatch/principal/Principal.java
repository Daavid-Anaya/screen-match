...
public class Principal {
    ...
    private void filtrarSeriesPorTemporadaYEvaluacion() {
        System.out.print("¿Filtrar séries con maximo de temporadas? ");
        var totalTemporadas = scanner.nextInt();
        scanner.nextLine();

        System.out.print("¿Con evaluación apartir de cuál valor? ");
        ...
        List<Serie> filtroSeries = serieRepository.seriesPorTemporadasYEvaluacion(totalTemporadas, evaluacion);
        System.out.println("*** Series filtradas ***");
        filtroSeries.forEach(s -> System.out.println(s.getTitulo() + "  - evaluacion: " + s.getEvaluacion()));
    }

    private void buscarEpisodiosPorTitulo() {
        System.out.print("Escribe el título del episodio que deseas buscar: ");
        var tituloEpisodio = scanner.nextLine();
        List<Episodio> episodiosEncontrados = serieRepository.episodiosPorNombre(tituloEpisodio);

        episodiosEncontrados.forEach(e -> System.out.printf("Serie: %s - Temporada: %s - Episodio: %s %s - Evaluación: %s\n",
    ...
    private void buscarTop5Episodios() {
        ...
        if (serieBuscada.isPresent()) {
            ...
            topEpisodios.forEach(e -> System.out.printf("Serie: %s - Temporada: %s - Episodio: %s %s - Evaluación: %s\n",
                e.getSerie().getTitulo(), e.getTemporada(), e.getNumeroEpisodio(), e.getTitulo(), e.getEvaluacion())
            );
        }
    }
